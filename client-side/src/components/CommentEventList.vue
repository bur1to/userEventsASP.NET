...
export default {
    ...
    methods: {
        getEventComments() {
            try {
                axios.get(`https://localhost:7219/EventComment/${localStorage.getItem('eventId')}`)
        ...
        deleteComment(id) {
            try {
                axios.delete(`https://localhost:7219/EventComment/${id}`);
            } catch (err) {
                console.log(err);
            }
        },
        updateComment(id) {
            try {
                axios.put(`https://localhost:7038/EventComment/${id}`);
            } catch (err) {
                console.timeLog(err);
            }
        },
